...
// Sets default values
ATreeSplinesISM::ATreeSplinesISM()
{
    ...
    DefaultTreeMeshPath = TEXT("/Game/CustomModels/trees2/pine_40.pine_40");

	// Set specific tree mesh paths
	Pine122_020_MeshPath = TEXT("/Game/CustomModels/trees2/pine_20.pine_20");

	Pine122_030_MeshPath = TEXT("/Game/CustomModels/trees2/pine_30.pine_30");

	Pine122_040_MeshPath = TEXT("/Game/CustomModels/trees2/pine_40.pine_40");

	Pine122_050_MeshPath = TEXT("/Game/CustomModels/trees2/pine_50.pine_50");

	Pine122_060_MeshPath = TEXT("/Game/CustomModels/trees2/pine_60.pine_60");

	Pine122_070_MeshPath = TEXT("/Game/CustomModels/trees2/pine_70.pine_70");

	Pine122_080_MeshPath = TEXT("/Game/CustomModels/trees2/pine_80.pine_80");

	Fir202_010_MeshPath = TEXT("/Game/CustomModels/trees2/fir_10.fir_10");
         
	Fir202_020_MeshPath = TEXT("/Game/CustomModels/trees2/fir_20.fir_20");

	Fir202_030_MeshPath = TEXT("/Game/CustomModels/trees2/fir_30.fir_30");

	Fir202_040_MeshPath = TEXT("/Game/CustomModels/trees2/fir_40.fir_40");

	Fir202_050_MeshPath = TEXT("/Game/CustomModels/trees2/fir_50.fir_50");

	Fir202_060_MeshPath = TEXT("/Game/CustomModels/trees2/fir_60.fir_60");

	Fir202_070_MeshPath = TEXT("/Game/CustomModels/trees2/fir_70.fir_70");
...
void ATreeSplinesISM::BeginPlay()
{
    Super::BeginPlay();
...
// Called every frame
void ATreeSplinesISM::Tick(float DeltaTime)
{
...
bool ATreeSplinesISM::LoadTreeMesh(int32 SPCD, float CR)
{
	if (!IsInGameThread())
	{
...
UOSMUserData* ATreeSplinesISM::GetOSMUserData(USplineComponent* SplineComponent)
{
...
FString ATreeSplinesISM::GetAppropriateTreeMeshPath(int32 SPCD, float CR)
{
    FString BasePath;
    TArray<FString*> PossiblePaths;

    // Determine the base path and possible paths based on SPCD
    if (SPCD == 122)
    {
        BasePath = TEXT("/Game/CustomModels/trees2/pine_");
        PossiblePaths = { &Pine122_020_MeshPath, &Pine122_030_MeshPath, &Pine122_040_MeshPath,
                          &Pine122_050_MeshPath, &Pine122_060_MeshPath, &Pine122_070_MeshPath, &Pine122_080_MeshPath };
    }
    else if (SPCD == 202)
    {
        BasePath = TEXT("/Game/CustomModels/trees2/fir_");
        PossiblePaths = { &Fir202_010_MeshPath, &Fir202_020_MeshPath, &Fir202_030_MeshPath,
                          &Fir202_040_MeshPath, &Fir202_050_MeshPath, &Fir202_060_MeshPath, &Fir202_070_MeshPath };
    }
    else if (SPCD == 66)
    {
        BasePath = TEXT("/Game/CustomModels/trees2/juniper_");
        PossiblePaths = { &Juniper66_010_MeshPath, &Juniper66_020_MeshPath, &Juniper66_030_MeshPath };
    }
    else
    {
    ...
    FString MeshPath = FString::Printf(TEXT("%s%d.%s%d"), *BasePath, ClosestCR, *BasePath.Replace(TEXT("/Game/CustomModels/trees2/"), TEXT("")), ClosestCR);

    // Check if the constructed path exists in PossiblePaths
    for (FString* Path : PossiblePaths)
    {
        if (*Path == MeshPath)
        {
            return MeshPath;
...
void ATreeSplinesISM::ProcessOSMUserData()
{
    UWorld* World = GetWorld();
    if (!World)
    {
        UE_LOG(LogTemp, Error, TEXT("ATreeSplinesISM::ProcessOSMUserData: World is null"));
        return;
    }
    ...
    for (TActorIterator<AActor> ActorItr(World); ActorItr; ++ActorItr)
    {
        ...
        for (USplineComponent* SplineComponent : SplineComponents)
        {
            ...
            if (OSMData)
            {
				...
				{
					UE_LOG(LogTemp, Log, TEXT("ATreeSplinesISM::ProcessOSMUserData: Unknown tree loaded"));
    ...
    UE_LOG(LogTemp, Log, TEXT("ATreeSplinesISM::ProcessOSMUserData: Added %d tree instances"), TreeInstances.Num());

}

void ATreeSplinesISM::ProcessOSMUserDataFromEditor()
{
#if WITH_EDITOR
    ProcessOSMUserData();
...
FTransform ATreeSplinesISM::GetTreeTransformAtSplineCenter(USplineComponent* SplineComponent, float Height, float Diameter)
{
    if (!TreeInstancedMesh->GetStaticMesh())
    {
        UE_LOG(LogTemp, Error, TEXT("ATreeSplinesISM::GetTreeTransformAtSplineCenter: TreeMesh is not set"));
...
//void ATreeSplinesISM::SpawnTreeAtSplineCenter(USplineComponent* SplineComponent, float Height, float Diameter)
//{
//    if (!TreeInstancedMesh->GetStaticMesh())
//    {
